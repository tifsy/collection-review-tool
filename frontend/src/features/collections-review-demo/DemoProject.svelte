<script>
  ...
  $: heroSub = p ? p.name.split(' · ').slice(1).join(' · ') : '';
  ...
  async function handleGenerateQueues() {
    ...
    if (!Number.isInteger(parsedQueueCount) || parsedQueueCount < 1) {
      queueGenerateError = 'Queue count must be a positive integer.';
      return;
    }

    if ((p?.queues?.length ?? 0) > 0) {
      queueGenerateError = 'Reviewer queues have already been generated.';
      return;
    }
    ...
    try {
      ...
      const result = await generateReviewProjectQueues(projectGuid, parsedQueueCount);
      ...
      queueGenerateError =
        error.response?.data?.error || error.message || 'Could not generate reviewer queues.';
    } finally {
  ...
  async function openProjectSettings() {
    ...
    settingsName = p.name;
    settingsShowVirtualQueueLinks = p.showVirtualQueueLinks;
    settingsEditMetadata = p.editMetadata;
    ...
    try {
      const result = await getReviewProjectGuidelines(projectGuid);

      localGuidelines = result.guidelines || '';
    } catch (error) {
      console.error(error);

      settingsError =
        error.response?.data?.error || error.message || 'Could not load project settings.';
    } finally {
  ...
  async function handleSaveSettings() {
    ...
    const trimmedGuidelines = localGuidelines.trim();
    ...
    if (!trimmedGuidelines) {
      settingsError = 'Annotation guidelines cannot be empty.';
      return;
    }
    ...
    try {
      ...
      await Promise.all([
        setReviewProjectName(projectGuid, trimmedName),

        setReviewProjectGuidelines(projectGuid, trimmedGuidelines),

        setReviewProjectReviewerLandingVirtualQueues(projectGuid, settingsShowVirtualQueueLinks),

        setReviewProjectEditMetadata(projectGuid, settingsEditMetadata),
      ]);
      ...
      settingsError =
        error.response?.data?.error || error.message || 'Could not save project settings.';
    } finally {
...
<div class="project-page">
    ...
    <div class="hero">
      ...
      <div class="hero-body">
        <div class="hero-left">
          <h1 class="hero-h1">
            {heroMain}{#if heroSub}
              <span class="mute">· {heroSub}</span>{/if}
          </h1>
          <div class="chips-row">
          ...
          <p class="about-tool">
            ...
            Seed collections are split into reviewer queues that you share by link. As reviewers decide
            each source, their
            <b class="about-kept"> Keep</b> / <b class="about-removed">Remove</b> /
            <b class="about-skipped">Skip</b> calls roll up here. When you're satisfied, publish the
            kept set back to a Media Cloud collection.
          </p>
        </div>
        <div class="hero-actions">
          ...
          <button class="btn btn-primary">
            <svg
              ...
              stroke-width="1.5"
              stroke-linecap="round"
              stroke-linejoin="round"><path d="M13 2 4 14h6l-1 8 9-12h-6z" /></svg
            >
            Publish
          </button>
        </div>
    ...
    <div class="section-pad">
      <div class="card">
        <div class="stats-header">
          <div>
            ...
            <div class="stats-count">
              {projectStats.decided.toLocaleString()}<span class="stats-total">
                / {projectStats.total.toLocaleString()}</span
              >
            </div>
          </div>
          <div class="stats-right">
            <div class="stats-undecided">
              <b class="mono-num">{projectStats.undecided.toLocaleString()}</b> undecided
            </div>
        ...
        <div class="stats-bar-wrap">
          <DecisionBar
            totals={{
              decided: projectStats.decided,
              kept: projectStats.kept,
              removed: projectStats.removed,
              added: projectStats.added,
              skipped: projectStats.skipped,
              undecided: projectStats.undecided,
            }}
            height={16}
            {highlight}
          />
          ...
          {#each DECISION_TILES as d}
            ...
            >
              ...
              <svg
                ...
                style:transition="color .2s"><path d="M7 17 17 7M9 7h8v8" /></svg
              >
            </button>
          {/each}
        </div>
        ...
        {#each p.queues as q, i}
          ...
          <div class="queue-card">
            <div class="queue-header">
              ...
              {#if isComplete}
                <span class="chip chip-skipped"
                  ><span class="chip-dot chip-dot-skipped"></span>Completed</span
                >
              {:else if isNew}
                <span class="chip chip-warn"
                  ><span class="chip-dot chip-dot-warn"></span>Unassigned</span
                >
              {/if}
            </div>
            ...
            <div class="queue-stats">
              <span class="qstat"
                ><b class="mono-num">{q.total}</b> <span class="mute">total</span></span
              >
            ...
            <div class="queue-footer">
              <button class="btn btn-sm" on:click={() => copyLink(q.url, i)}>
                ...
                {:else}
                  <svg
                    ...
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    ><rect x="8" y="8" width="13" height="13" rx="2" /><path
              ...
              <button
                ...
                on:click={() => onNavigate(`/demo/review-projects/${projectGuid}/queues/${q.guid}`)}
              >
                <svg
                  width="11"
                  height="11"
  ...
  {:else}
    <div class="not-seeded">
      ...
      <p class="not-seeded-body">
        This project exists in the demo but hasn't been set up with detailed mock data yet.
      </p>
                  ...
                  <div class="verdict-chips">
                    {#each ['kept', 'removed', 'added', 'skipped'] as v}
                      <button
                        ...
                        on:click={() => (bucketNewVerdict = v)}>{VERDICT_LABELS[v]}</button
                      >
                    {/each}
                  </div>
...
{#if showGenerateQueue && p}
  ...
  >
    <div class="modal" on:click|stopPropagation>
      <div class="modal-header">
        <div>
          <div class="modal-title">Generate reviewer queues</div>
      ...
      <div class="modal-body">
        {#if settingsLoading}
          <div class="setting-desc" style="margin-bottom: 12px;">Loading project settings...</div>
        {/if}

        {#if settingsError}
          <div class="setting-desc" style="color: #b42318; margin-bottom: 12px;" role="alert">
            {settingsError}
          </div>
        {/if}
        ...
        <div class="setting-row">
          <div class="setting-info">
            <div class="setting-title">Number of queues</div>

            <div class="setting-desc">Sources will be distributed as evenly as possible.</div>
          </div>
        ...
        {#if queueGenerateError}
          <div class="setting-desc" style="color: #b42318; margin-top: 12px;">
            {queueGenerateError}
          </div>
        {/if}
      </div>
    ...
    <div class="modal" on:click|stopPropagation>
      <div class="modal-header">
        <div>
          ...
          <div class="modal-subtitle">
            Change what reviewers see or whether they can edit source metadata.
          </div>
        </div>
        <button
          class="modal-close"
        ...
        <div class="setting-row setting-row-toggle">
          <div class="setting-info">
            ...
            <div class="setting-desc">
              Show project-wide virtual-queue links on reviewer landing pages.
            </div>
          </div>
          <button
            type="button"
            ...
            on:click={() => {
              settingsShowVirtualQueueLinks = !settingsShowVirtualQueueLinks;
            }}
          >
            <div class="toggle-knob" class:toggle-knob-on={settingsShowVirtualQueueLinks}></div>
          </button>
        </div>
        <div class="setting-row setting-row-toggle">
          <div class="setting-info">
            ...
            <div class="setting-desc">
              Require reviewers to confirm language and country/state before keeping.
            </div>
          </div>
          <button
            type="button"
            ...
            on:click={() => {
              settingsEditMetadata = !settingsEditMetadata;
            }}
          >
            <div class="toggle-knob" class:toggle-knob-on={settingsEditMetadata}></div>
          </button>
        </div>
      </div>
      <div class="modal-footer">
...
<style>
  ...
  .modal {
    ...
    border-radius: 18px;
    border: 1px solid var(--v2-line);
    box-shadow: 0 30px 70px -24px rgba(20, 23, 30, 0.45);
    overflow: hidden;
    flex-shrink: 0;
  }
  .modal-header {
