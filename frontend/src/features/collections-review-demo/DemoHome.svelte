<script>
  ...
  async function submitNewProject() {
    ...
    try {
      ...
      onNavigate(`/demo/review-projects/${result.project.guid}`);
    } catch (error) {
      console.error(error);

      newProjectError =
        error.response?.data?.error || error.message || 'Could not create the project.';
    } finally {
      newProjectCreating = false;
    }
  }
  ...
  <div class="hero">
    <div class="hero-grid">
      ...
      <div class="hero-left">
        ...
        <p class="hero-body">
          Curate Media Cloud's news-source collections: keep what holds up, remove the noise,
          publish back when you're done.
        </p>
      ...
      <div class="qrc">
        ...
        <div class="qrc-options">
          <div class="qrc-option-box">
            ...
            <button
              class="qrc-option-val-row"
              on:click={() => (metadataEditing = !metadataEditing)}
            >
              <span class="qrc-option-val">{metadataEditing ? 'On' : 'Off'}</span>
              <div
                class="toggle"
                class:toggle-on={metadataEditing}
                class:toggle-off={!metadataEditing}
              >
      ...
      {#each $inProgressProjects.slice(0, 5) as r, i}
        ...
        >
          <div class="review-info">
            ...
            <div class="review-id">
              {r.seeds} seed collections · {r.queueCount}
              {r.queueCount === 1 ? 'queue' : 'queues'}
            </div>
          </div>
...
<Modal
  show={showNewProject}
  title="Start review project"
  on:close={() => {
    if (!newProjectCreating) showNewProject = false;
  }}
>
  ...
  <form class="modal-form" on:submit|preventDefault={submitNewProject}>
    ...
    {#if newProjectError}
      <div class="form-hint" style="color: #b42318;" role="alert">
        {newProjectError}
      </div>
    {/if}
    ...
    >
      ...
      {newProjectCreating ? 'Creating project...' : 'Start review project'}
    </button>
  </form>
</Modal>
...
<!-- All in-progress -->
<Modal
  show={showAllInProgress}
  title="In-progress reviews"
  on:close={() => (showAllInProgress = false)}
>
...
<!-- All completed -->
<Modal
  show={showAllCompleted}
  title="Completed reviews"
  on:close={() => (showAllCompleted = false)}
>
  <div class="modal-list">
    {#each $completedProjects as r, i}
      <div class="modal-row" class:first={i === 0}>
        ...
        <div class="modal-row-right">
          <span class="chip chip-skipped"
            ><span class="chip-dot chip-dot-skipped"></span>Closed</span
          >
        </div>
      </div>
    {/each}
...
<style>
  ...
  .qrc {
    ...
    border-radius: 16px;
    padding: 0;
    box-shadow: 0 18px 50px -28px rgba(0, 0, 0, 0.16);
    overflow: hidden;
  }
  .qrc-header {
    padding: 14px 20px;
  ...
  .radio-option:hover {
    background: var(--v2-line-soft, #f5f5f7);
  }
  .radio-selected {
    border-color: var(--v2-accent);
    background: var(--v2-accent-soft, #fdf1ee);
  }
  .radio-text {
    display: flex;
    flex-direction: column;
  ...
  .modal-row {
    ...
    gap: 12px;
    padding: 13px 18px;
    border-top: 1px solid var(--v2-line-soft, #f0f0f0);
    cursor: pointer;
    background: transparent;
    border-left: none;
    border-right: none;
  ...
  .modal-row:is(button):hover {
    background: #f8f8f8;
  }
  .modal-row .review-info {
    flex: 1;
    min-width: 0;
  }
  .modal-row-right {
